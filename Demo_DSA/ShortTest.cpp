...
void testAll() { //call each function to see if it is implemented
	...
	std::pair<TValue, TValue> res1 = m.removeKey(10);
	std::pair<TValue, TValue> ans12(110, 10);
	std::pair<TValue, TValue> ans22(10, 110);
	std::pair<TValue, TValue> ans32(NULL_TVALUE, NULL_TVALUE);
	assert(((res1 == ans12) || (res1 == ans22)));

	assert(m.add(10, 10) == true);
	std::pair<TValue, TValue> ans13(10, NULL_TVALUE);
	std::pair<TValue, TValue> res2 = m.removeKey(10);
	assert(res2 == ans13);
	std::pair<TValue, TValue> res3 = m.search(10);
	assert(res3 == ans32);
